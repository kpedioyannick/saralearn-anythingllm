...
export function parseRichBlocks(html) {
  ...
  for (const node of root.childNodes) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE && cls) {
      if (cls.contains("quiz-block") || cls.contains("dictee-block") || cls.contains("geogebra-block") || cls.contains("markmap-block") || cls.contains("probleme-block") || cls.contains("video-block") || cls.contains("video-url-block") || cls.contains("video-preview-block") || cls.contains("h5p-block")) {
        ...
        const type = cls.contains("quiz-block") ? "quiz"
          ...
          : cls.contains("video-preview-block") ? "video-preview"
