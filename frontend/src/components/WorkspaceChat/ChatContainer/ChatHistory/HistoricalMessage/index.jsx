    ...
    return (
      ...
      >
        <div className="py-4 px-4 flex flex-col items-end">
          <div className="bg-[rgba(17,140,68,0.22)] light:bg-[rgba(17,140,68,0.1)] border border-[rgba(74,222,128,0.35)] light:border-[rgba(17,140,68,0.25)] rounded-[16px] rounded-br-none px-3 py-3 md:px-4 md:py-3.5 max-w-[600px] [&_p]:m-0 shadow-[0_6px_18px_rgba(0,0,0,0.18)]">
            <TruncatableContent>
              <RenderChatContent
                role={role}
                message={message}
...
const RenderChatContent = memo(
  ({ role, message, messageId, workspace, activeThread }) => {
    if (role !== "assistant")
      return (
        <span
    ...
    return (
      <>
        ...
        <RichMessageRenderer message={msgToRender} workspace={workspace} activeThread={activeThread} />
      </>
    );
  },
  (prevProps, nextProps) => {
    return (
      ...
      prevProps.activeThread?.id === nextProps.activeThread?.id
    );
  }
);
