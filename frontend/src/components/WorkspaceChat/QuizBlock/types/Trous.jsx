...
export default function Trous({
  ...
  const submit = () => {
    const correct = blanks.every(
      (b, i) => inputs[i].trim().toLowerCase() === b.toLowerCase()
    );
    onAnswer(correct);
  };
          ...
          <React.Fragment key={i}>
            ...
            {i < blanks.length &&
              (answered === undefined ? (
                ...
                <span
                  className={`inline-block px-2 py-0.5 rounded text-xs font-semibold ${inputs[i].trim().toLowerCase() === blanks[i].toLowerCase() ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}
                >
                  {inputs[i] || "—"}{" "}
                  {inputs[i].trim().toLowerCase() !== blanks[i].toLowerCase() &&
                    `(→ ${blanks[i]})`}
                </span>
              ))}
          </React.Fragment>
