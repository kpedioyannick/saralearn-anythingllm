// TrousRC : même rendu que Trous (réponse courte tapée), mais correction tolérante (sous-chaîne)
import React, { useState } from "react";
import { useTranslation } from "react-i18next";

export default function TrousRC({ segments, blanks, onAnswer, answered }) {
  const { t } = useTranslation();
  const [inputs, setInputs] = useState(Array(blanks.length).fill(""));

  const isMatch = (input, blank) =>
    blank.toLowerCase().includes(input.trim().toLowerCase()) && input.trim().length > 0;
          ...
          <React.Fragment key={i}>
            ...
            {i < blanks.length && (
              answered === undefined ? (
                ...
                <span className={`inline-block px-2 py-0.5 rounded text-xs font-semibold ${isMatch(inputs[i], blanks[i]) ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}>
                  {inputs[i] || "—"} {!isMatch(inputs[i], blanks[i]) && `(→ ${blanks[i]})`}
                </span>
              )
            )}
          </React.Fragment>
